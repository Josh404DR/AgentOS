...
function New-CodexVerifyTask {
    ...
    $changeRequiredMatch = [regex]::Match(
        $parentResult,
        '(?mi)^change_required\s*:\s*(true|false)\s*$'
    )
    ...
    $testEvidence = @(
        [regex]::Matches($parentResult, '(?mi)^(test_command|test_result|verification)\s*:\s*(.+)$') |
...
switch ($RouteTo) {
    "Codex" {
        ...
        if (-not $DryRun) {
            ...
            $psi.Arguments = '/d /c ""' + $codexCommand + '" -a never exec -C "' + $AgentOSRoot +
    ...
    "Claude" {
        ...
        $commandDescription = "claude -p --permission-mode acceptEdits --no-session-persistence < `"$promptPath`""
        if (-not $DryRun) {
            $psi = [Diagnostics.ProcessStartInfo]::new()
            $psi.FileName = "cmd.exe"
            $psi.WorkingDirectory = $AgentOSRoot
            ...
            $psi.StandardOutputEncoding = [Text.Encoding]::UTF8
            $psi.StandardErrorEncoding = [Text.Encoding]::UTF8
            $psi.Arguments = '/d /c claude -p --permission-mode acceptEdits --no-session-persistence < "' + $promptPath + '"'
            $process = [Diagnostics.Process]::new()
            $process.StartInfo = $psi
            [void]$process.Start()
            $stdoutTask = $process.StandardOutput.ReadToEndAsync()
