...
$isInfoQuery = $taskType -eq "INFO_QUERY"
$taskTitle = switch ($taskType) {
    ...
    "INFO_QUERY" { "Claude Info Query Task" }
...
$routeTo = if ($isPlanTask) { "Codex" } elseif ($taskType -in @("Simple", "INFO_QUERY")) { "Claude" } else { "STOP" }
$assignedTo = if ($isPlanTask) { "Codex" } elseif ($taskType -in @("Simple", "INFO_QUERY")) { "Claude Worker" } else { "Josh" }
$packetType = if ($isPlanTask) { "CODEX_PLAN" } elseif ($taskType -in @("Simple", "INFO_QUERY")) { "CLAUDE_WORKER" } else { "ESCALATION" }
...
$infoQueryInstructions = if ($isInfoQuery) {
@"
## Info Query Contract

- This is a pure information request. Do not modify any workspace file.
- Use WebSearch/WebFetch to obtain current information; cite sources.
- Emit `change_required: false` and zero `changed_file` lines.
- Provide at least one line of:
  evidence: <source URL or lookup method>
"@
} else { "" }
...
This line must be plain text on its own line: no bold, no markdown
decorations, and no trailing note on the same line.

For every verification command, include:
test_command: <literal command>
test_result: <PASS|FAIL and concise evidence>

Query-type exception: if and only if `change_required: false` and there are
zero `changed_file` lines, you may replace test_command/test_result with at
least one line of:
evidence: <source URL or lookup method>
Implementation tasks (any changed_file present) must still provide
test_command and test_result.

$infoQueryInstructions
$planInstructions
"@
...
$agentRole = if ($isPlanTask) {
    ...
    "You are Claude Worker executing an AgentOS info query task. Answer the information request using WebSearch/WebFetch with cited sources. Do not modify any workspace file."
} else {
