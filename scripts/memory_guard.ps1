...
if ($KillCandidates) {
    ...
    foreach ($candidate in $candidates) {
        try {
            ...
            Write-Output "KILL_FAILED=$($candidate.ProcessName) PID=$($candidate.Id) ERROR=$($_.Exception.Message)"
        }
    }
} else {
    Write-Output ""
    Write-Output "KILL_MODE=dry_run"
    Write-Output "NOTE=No processes were closed. Re-run with -KillCandidates only after reviewing CLOSE_CANDIDATES."
}
