...
Local smoke runs invoke `scripts\model_cli_health_smoke.ps1` by default. Codex,
Claude, and Antigravity must each pass a minimal real model request; binary
presence or `--version` alone is not sufficient. The receipt records the
resolved executable, CLI version, invocation exit code, identity, and a
credential-redacted failure category in `data\ci_health\model-cli-latest.json`.

`-SkipModelCliSmoke` is only for hosted CI environments without local model
credentials. It produces `WARN`, never `PASS`, for model availability. Release,
workflow, runner, and smoke-task acceptance on an AgentOS workstation must not
use that switch.
