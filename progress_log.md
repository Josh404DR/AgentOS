...
## 2026-07-29 - Governance Change Approval: EVIDENCE_AND_REPORTING_CONTRACT.md Section 7 Role Realignment
### [STATUS: approved_by_josh]

- **Context**: `2026-07-29-evidence-contract-baseline-drift-check` (read-only Codex verify) found 4 uncommitted semantic differences between the working copy of `docs\EVIDENCE_AND_REPORTING_CONTRACT.md` and its last committed git version (`961a83a59c593371c0fdcdef6c2ab193cca244af`), tracked as F11 in `docs\PROJECT_FINDINGS_REGISTRY.md`.
- **Changes approved for commit**:
  - Line 290: Codex role title changed from "Builder/Technical Executor" to "Complex Planner / Blind Verifier".
  - New Mode Isolation rule (line ~295): a Codex Builder delivery must be verified by a different fresh Codex Verify process/session; Planner/Builder/Verifier are execution-mode labels, not exclusive permanent vendor assignments.
  - Line 301: Claude role title changed from "Inspector" to "Workspace Implementation / Revision Worker".
  - Line 305: "Advisory Only" rule replaced with "Implementation Is Not Verification" ??Claude delivery does not replace an independent Codex Blind Verify verdict or Josh approval.
  - (Also present in working copy, not separately itemized by the drift-check but part of the same uncommitted diff: Section 3's full/lightweight Evidence Block tiering, and the Section 4 resource-contribution-summary block.)
- **Action**: Josh approved committing these changes as-is (Cowork conversation, 2026-07-29) ??per the contract's own Section 2 rule "Governance Change: Any change to this contract requires explicit Josh approval and a `progress_log.md` entry", this entry satisfies that requirement.
- **Next Step**: `git add docs\EVIDENCE_AND_REPORTING_CONTRACT.md progress_log.md && git commit -m "docs: commit EVIDENCE_AND_REPORTING_CONTRACT.md role realignment (F11, Josh-approved 2026-07-29)"` ??Claude has no PowerShell/git execution in this session; Josh or Codex must run this.
